...
const SITE_CONFIG_PATH = path.resolve('src/config/siteConfig.ts');
const CONCURRENCY = 8;
const RETRIES = 3;
...
/**
 * Parse playlists from siteConfig.ts using regex.
 * Looks for the music.playlists array pattern.
 */
async function parsePlaylistsFromConfig() {
  try {
    const content = await fs.readFile(SITE_CONFIG_PATH, 'utf-8');

    // Extract the playlists array content between [ and ]
    const playlistsMatch = content.match(/playlists\s*:\s*\[([\s\S]*?)\]/);
    if (!playlistsMatch) {
      console.log('No playlists found in siteConfig.ts, using defaults');
      return [];
    }

    const arrayContent = playlistsMatch[1];
    const entries = [];

    // Match each playlist entry: { id: "...", name: "...", ... }
    const entryReg = /\{\s*id\s*:\s*["']([^"']+)["']\s*,\s*name\s*:\s*["']([^"']+)["']\s*(?:,\s*server\s*:\s*["']([^"']+)["'])?\s*(?:,\s*type\s*:\s*["']([^"']+)["'])?\s*\}/g;
    let match;
    while ((match = entryReg.exec(arrayContent)) !== null) {
      entries.push({
        id: match[1],
        name: match[2],
        server: match[3] || 'netease',
    ...
    console.error('Failed to parse siteConfig.ts:', e.message);
    return [];
  }
}
    ...
    try {
      ...
      if (item.url && item.url.includes('163.hyc.moe')) {
        try {
          ...
          // Netease API failed, fall through to buffer method
        }
      }
...
async function fetchMusicDuration() {
  try {
    const trans = true; // always fetch translations
    const playlists = await parsePlaylistsFromConfig();

    if (playlists.length === 0) {
      console.log('No playlists configured. Add playlists to siteConfig.ts music.playlists');
      return;
    }
    ...
    for (const pl of playlists) {
      let songs;
      if (pl.type === 'custom') {
        // Custom playlist - read from existing music.json
        try {
          songs = existingData.playlistSongs?.[pl.id] || [];
          console.log(`  ${pl.name || pl.id} (custom): ${songs.length} songs`);
        } catch (e) {
          console.error(`  Failed to read custom playlist ${pl.name || pl.id}:`, e.message);
          songs = [];
        }
      } else {
    ...
    if (pending.length === 0) {
      ...
      // Still save to ensure format is up to date
    ...
    if (failed > 0) {
      console.log(`${failed} songs could not get durations. Re-run to retry.`);
    }
  } catch (error) {
