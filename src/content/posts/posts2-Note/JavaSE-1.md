...
pinned: false
image: 'https://img.eysnter.cn/file/posts/posts2-notes/JavaSE/1780418879686_JavaSE-1.jpg'
tags: [JavaSE]
category: '技术参考'
draft: false
description: 这是java笔记
lang: zh-CN
comment: true
---
...
2. **必须初始化**：变量在使用（如打印、参与运算）之前，**必须先赋值**，否则编译报错。
3. **类型约束**：变量只能存储符合其定义时指定的数据类型的数据。
...
## 五. 标识符命名规范 (Identifiers)

### 1. 硬性要求（不遵守则编译报错 ❌）

1. 只能由 **数字、字母、下划线 `_`、美元符号 `$`** 组成。 
2. **绝对不能以数字开头**。
3. **不能是 Java 关键字**（如：不能用 `class`、`public`、`void` 做变量名）。
4. **严格区分大小写**（如 `username` 和 `userName` 是两个不同的变量）。
### 2. 软性规范（不遵守显 low，行业统一标准 💎）
...
## 八. 算术运算符与数值拆分 (Arithmetic Operators)
...
- **`/` (除法) 陷阱**：
    ...
    2. 如果想要得到小数结果，必须有浮点数（小数）参与运算。
    3. 浮点数直接运算可能得到不精确的结果（例如 `10.0 / 3` 在 Java 中输出 `3.3333333333333335`）。
